...
async function prove(args) {
    ...
    const signature = (0, util_1.toHexString)(eddsa.packSignature(eddsa.signPoseidon(prvKey, hashedMessage)));
...
async function verify(pcd) {
    try {
        await ensureInitialized();
        const signature = eddsa.unpackSignature((0, util_1.fromHexString)(pcd.proof.signature));

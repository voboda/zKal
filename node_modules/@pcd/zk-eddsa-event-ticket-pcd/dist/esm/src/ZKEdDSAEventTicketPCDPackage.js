...
import { EdDSATicketPCDPackage, EdDSATicketPCDTypeName, ticketDataToBigInts } from "@pcd/eddsa-ticket-pcd";
import { ArgumentTypeName } from "@pcd/pcd-types";
import { SemaphoreIdentityPCDPackage } from "@pcd/semaphore-identity-pcd";
import { STATIC_SIGNATURE_PCD_NULLIFIER } from "@pcd/semaphore-signature-pcd";
import { BABY_JUB_NEGATIVE_ONE, babyJubIsNegativeOne, booleanToBigInt, decStringToBigIntToUuid, fromHexString, generateSnarkMessageHash, hexToBigInt, numberToBigInt, requireDefinedParameter, uuidToBigInt } from "@pcd/util";
import { buildEddsa } from "circomlibjs";
import JSONBig from "json-bigint";
import { groth16 } from "snarkjs";
import { v4 as uuid } from "uuid";
import vkey from "../artifacts/circuit.json";
import { VALID_EVENT_IDS_MAX_LEN, ZKEdDSAEventTicketPCD, ZKEdDSAEventTicketPCDTypeName } from "./ZKEdDSAEventTicketPCD";
...
async function checkProveInputs(args) {
    ...
    const deserializedTicket = await EdDSATicketPCDPackage.deserialize(serializedTicketPCD);
...
function snarkInputForProof(ticketPCD, identityPCD, fieldsToReveal, validEventIdsInput, externalNullifer, watermark) {
    ...
    const rawSig = eddsa.unpackSignature(fromHexString(ticketPCD.proof.eddsaPCD.proof.signature));
    ...
    return {
        ...
        // Security features
        externalNullifier: externalNullifer || STATIC_TICKET_PCD_NULLIFIER.toString(),
            ...
            ticket: {
                ...
                validate(value, params) {
                    ...
                    if (params?.publicKeys?.length) {
                        if (!params.publicKeys.find((publicKey) => isEqualEdDSAPublicKey(publicKey, value.proof.eddsaPCD.claim.publicKey))) {
                            return false;
                        }
                    }
                    return true;
...
export async function prove(args) {
    ...
    const { proof, publicSignals } = await groth16.fullProve(snarkInput, initArgs.wasmFilePath, initArgs.zkeyFilePath);
...
export async function deserialize(serialized) {
    const { id, claim, proof } = JSONBig({ useNativeBigInt: true }).parse(serialized);
...
export function isZKEdDSAEventTicketPCD(pcd) {
 ...
 */
export const ZKEdDSAEventTicketPCDPackage = {

...
async function generateProof(identity, groupOrMerkleProof, message, scope, merkleTreeDepth, snarkArtifacts) {
    errorHandlers.requireDefined(identity, "identity");
    errorHandlers.requireDefined(groupOrMerkleProof, "groupOrMerkleProof");
    errorHandlers.requireDefined(message, "message");
    errorHandlers.requireDefined(scope, "scope");
    errorHandlers.requireObject(identity, "identity");
    errorHandlers.requireObject(groupOrMerkleProof, "groupOrMerkleProof");
    errorHandlers.requireTypes(message, "message", ["string", "bigint", "number", "Uint8Array"]);
    errorHandlers.requireTypes(scope, "scope", ["string", "bigint", "number", "Uint8Array"]);
    if (merkleTreeDepth) {
        errorHandlers.requireNumber(merkleTreeDepth, "merkleTreeDepth");
    }
    if (snarkArtifacts) {
        errorHandlers.requireObject(snarkArtifacts, "snarkArtifacts");
    }
    ...
    snarkArtifacts ?? (snarkArtifacts = await artifacts.maybeGetSnarkArtifacts(artifacts.Project.SEMAPHORE, { parameters: [merkleTreeDepth] }));
    ...
    return {
        ...
        points: proofPacking.packGroth16Proof(proof)
    };
}
